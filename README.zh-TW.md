...
移除「7 天過期」只是半套。level 只有在 K 線窗內才看得見，所以**窗深就是實際的可追溯期限**：
...
引擎當下沒回傳的 28 筆，逐一分類原因：

| 原因 | 筆數 | 判定 |
|---|---|---|
| 已被更極端的 pivot 取代（既有結構失效規則） | 26 | ✅ 正確行為 |
| 已穿越但事件未入帳（真漏報） | 0 | ✅ 沒有漏報 |
| 有效、未穿越，被引擎「分數前 20 名」截掉 | 2 | ⚠️ 已知限制 |

那 2 筆分別是 18 天與 20 天前的價位。引擎的分數含年齡衰減（1H 半衰期 200 根 ≈ 8 天），老價位會在排名競爭中落敗而被擠出。**這不是「7 天規則」，但效果類似**——已在程式碼註解標明，未擅自更動引擎。

`candle-depth.test.ts` 守住窗深：把 `SCAN_CANDLE_LIMIT` 調小到涵蓋不足 7 天就會測試失敗。

複現：

```bash
# 逐幣核對帳本未解決 level 與引擎輸出的落差，並分類原因
