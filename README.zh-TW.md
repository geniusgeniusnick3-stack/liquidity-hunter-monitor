...
把原本的單幣網頁儀表板，改造成**在 Telegram 上查詢**的全市掃描器，
且可選用背景主動監控。
...
用 SMC 引擎分析它們的 BSL / SSL 流動性，並在你下指令時（ACTIVE 模式下則於 K 線收盤時）
回報值得人工查看的價位事件。
...
| 通知 | 無 | Telegram —— PASSIVE 模式為隨查隨回，ACTIVE 模式為狀態轉變主動推播 |
| 網頁儀表板 | 有 | **維持原狀，本專案不使用** |
...
這些門檻是目前依 Binance USDT-M 市場實際分布校準出來的預設值，可自行調整，
不宣稱對所有人都是最佳值。
...
| 背景監控為明示選用 | **PASSIVE 是預設**，本身不持續掃描任何市場。**ACTIVE 為選用**：持續監控與主動推播，只在使用者明確啟用 ACTIVE 模式後才會開始 |
